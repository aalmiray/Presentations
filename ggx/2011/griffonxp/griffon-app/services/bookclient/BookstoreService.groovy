...
class BookstoreService {
  ...
  void populateModel(model) {
    withRest(id: 'bookstoreREST', uri: 'http://localhost:8081/bookstore/'){
      ...
      execSync { model.authors.clear(); model.authors.addAll(authors) }

      response = get(path: 'book')
      def books = response.data.collect([]) { book ->
        [title: book.title, id: book.id]
      }
      execSync { model.books.clear(); model.books.addAll(books) }
   }}
}
